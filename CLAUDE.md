...
- The coordinator is the main-session role with normal runtime permissions; do not launch it as a background coordinator. Each independent session uses the user-configured model; do not assume it inherits the coordinator’s model. Tool availability does not bypass permissions.
...
- Native `Agent` is restricted to role-free read-only discovery (`Agent(Explore)` in the coordinator's tools), never specialist roles. Do not use `--resume`, `--continue` or `--fork-session` for assignments.
