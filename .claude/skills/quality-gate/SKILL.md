...
- Write only the assigned receipt file, confirming `profile_read` and `instructions_read` as absolute paths actually read. Never modify target files. Return the six-field receipt (`status`, `changed`, `checks`, `evidence`, `risks`, `next`). Keep review verdict separate: `FAIL` for evidenced blocking defects, `PASS_WITH_RISKS` for material limitations/risks without a confirmed blocking defect, `PASS` when reviewed scope has no blocking findings or material remaining risk. A completed review can have `status: done` and verdict `FAIL`.
- Send corrections back through the coordinator to the same role in a new correction session; do not start another implementation or claim global safety from a bounded review.
